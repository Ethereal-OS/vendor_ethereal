...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop org.ethereal.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/Ethereal/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.ethereal.device | grep -q "$CUSTOM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop org.ethereal.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
