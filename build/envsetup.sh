...
cat <<EOF
Additional EtherealOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- etherealgerrit:   A Git wrapper that fetches/pushes patch from/to EtherealOS Gerrit Review.
- etherealrebase:   Rebase a Gerrit change and push it again.
- etherealremote:   Add git remote for EtherealOS Gerrit Review.
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/ethereal/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch ethereal_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
function etherealrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "EtherealOS Gerrit Rebase Usage: "
