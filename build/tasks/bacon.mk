...
bacon: $(DEFAULT_GOAL) $(INTERNAL_OTA_PACKAGE_TARGET)
	...
	echo -e ${CL_BLD}${CL_GRN}"Size:"${CL_RED}" `du -sh $(ETHEREAL_TARGET_PACKAGE) | awk '{print $$1}' `"${CL_RST}
	echo -e ${CL_BLD}${CL_GRN}"TimeStamp:"${CL_RED}" `cat $(PRODUCT_OUT)/system/build.prop | grep ro.ethereal.build.date | cut -d'=' -f2 | awk '{print $$1}' `"${CL_RST}
