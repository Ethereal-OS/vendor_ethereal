...
# this can be done before initiate build environment (. build/envsetup.sh)
if [ -z $changelog_days ];then
    changelog_days=10
else
    if (($changelog_days > 30 )); then
        echo "Changelog cannot be generated for more than 30 days. For how many days do you want to generate changelog again? (🕑 timeout 15 seconds - default to 10 days)"
        read -r -t 15 changelog_days || changelog_days=10
    fi
fi

REPO_LIST="$(cat .repo/project.list | sed '\?^vendor/officialdevices?d')"
for i in $(seq $changelog_days); do
    After_Date=$(date -d "$i days ago" +"%m-%d-%Y")
    k=$(expr $i - 1)
    Until_Date=$(date -d "$k days ago" +"%m-%d-%Y")
